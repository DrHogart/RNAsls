...
while ( my $seq = $seq_in->next_seq ) {
    ...
    # collect the results
    push @resulted_table, @add_randfold;
...
sub RNALfold_parsing {
    ...
    for ( my $j = 0; $j <= $#strs - 1; $j++ ) {
        if ( $strs[$j] =~ m/^[\.\(\)]/ ) {
            ...
            push @parsed_results,
                join( "\t", $name . '_' . $j, $str, $dG, $start, $Seq );
        }
    }
...
sub distance_measure {
    ...
    for ( my $i = 0; $i <= $#strs; $i++ ) {
        ...
        for ( my $j = 0; $j <= $#QuerySeqs; $j++ ) {
            ...
            open( FOREST, ">$ForesterFile" )
                || die "Can't open $ForesterFile: $!\n";
            print FOREST ">Query\n$QuerySeqs[$j]\n$SS_Seqs[$j]\n";
            print FOREST ">ID\n$sequence\n$str\n";
            close FOREST || die "Can't close $ForesterFile: $!\n";

            my $Forest_Out_1
                = `RNAforester --score --noscale -l < $ForesterFile`;
            chomp($Forest_Out_1);
            $Forest_Out_1 =~ s/ //g;
        ...
        push @resulted_distance,
            sprintf(
            "REP: ID=%s Length=%-3d Start=%-3d Score=%s-%s-%s-%s dG=%s",
            $ID, length($sequence), $start, $DistScore[0], $DistScore[1],
            $DistScore[2], $DistScore[3], $dG );
...
sub distance_filtering {
    ...
    for ( my $i = 0; $i <= $#strs; $i += 3 ) {
        my ( $rep, $ID, $size, $start, $scores, $dG )
            = split( /\s+/, $strs[$i] );
